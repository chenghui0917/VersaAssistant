...
struct DockWidgetPrivate
{
	...
	/**
	 * Show dock frameLessWidget
	 */
	void showDockWidget();

	/**
	 * Hide dock frameLessWidget.
	 */
	void hideDockWidget();
	...
	/**
	 * Creates the content frameLessWidget with the registered frameLessWidget factory and
...
void DockWidgetPrivate::showDockWidget()
{
	if (!Widget)
	{
		if (!createWidgetFromFactory())
		{
			Q_ASSERT(!Features.testFlag(CDockWidget::DeleteContentOnClose)
					 && "DeleteContentOnClose flag was set, but the frameLessWidget "
	...
	if (!DockArea)
	{
		...
		// We use the size hint of the content frameLessWidget to provide a good
		// initial size
	...
	else
	{
        ...
        // If this frameLessWidget is pinned and there are no opened dock widgets, unpin the auto hide frameLessWidget by moving it's contents to parent container
...
void DockWidgetPrivate::updateParentDockArea()
{
	...
	// current dock frameLessWidget is not the one being closed
...
void CDockWidget::toggleView(bool Open)
{
	...
	// If the dock frameLessWidget state is different, then we really need to toggle
	// the state. If we are in the right state, then we simply make this
	// dock frameLessWidget the current dock frameLessWidget
...
void CDockWidget::toggleViewInternal(bool Open)
{
	...
	// this dock frameLessWidget was unassigned before the call to showDockWidget() then
...
{
	s.writeStartElement("VersaWidget");
...
bool CDockWidget::closeDockWidgetInternal(bool ForceClose)
{
	...
	if (features().testFlag(CDockWidget::DockWidgetDeleteOnClose))
	{
		// If the dock frameLessWidget is floating, then we check if we also need to
		// delete the floating frameLessWidget
		if (isFloating())
		{
			CFloatingDockContainer* FloatingWidget = internal::findParent<
