...
struct DockWidgetTabPrivate
{
	...
	/**
	 * Starts floating of the dock frameLessWidget that belongs to this title bar
...
bool DockWidgetTabPrivate::startFloating(eDragState DraggingState)
{
	...
	// if this is the last dock frameLessWidget inside of this floating frameLessWidget,
	...
	// If section frameLessWidget has multiple tabs, we take only one tab
	// If it has only one single tab, we can move the complete
	// dock area into floating frameLessWidget
	QSize Size;
...
void CDockWidgetTab::mouseReleaseEvent(QMouseEvent* ev)
{
	...
	else if (ev->button() == Qt::MiddleButton)
	{
		if (CDockManager::testConfigFlag(CDockManager::MiddleMouseButtonClosesTab) && d->DockWidget->features().testFlag(CDockWidget::DockWidgetClosable))
		{
			...
			// on top of the frameLessWidget, to allow the user to cancel.
...
void CDockWidgetTab::mouseMoveEvent(QMouseEvent* ev)
{
    ...
    if (DragDistanceY >= CDockManager::startDragDistance() || MouseOutsideBar)
	{
    	...
    	// one single dock frameLessWidget it does not make  sense to move it to a new
    	// floating frameLessWidget and leave this one empty
		...
		// We can create the drag preview if the frameLessWidget is movable.
...
void CDockWidgetTab::mouseDoubleClickEvent(QMouseEvent *event)
{
	if (event->button() == Qt::LeftButton) 
	{
		...
		// sense to move it to a new floating frameLessWidget and leave this one
		// empty
