...
protected:
	/**
	 * Registers the given floating frameLessWidget in the internal list of
	 * floating widgets
	 */
	void registerFloatingWidget(CFloatingDockContainer* FloatingWidget);

	/**
	 * Remove the given floating frameLessWidget from the list of registered floating
	 * widgets
	 */
	void removeFloatingWidget(CFloatingDockContainer* FloatingWidget);

	/**
	 * Registers the given dock container frameLessWidget
	 */
	...
	/**
	 * A container needs to call this function if a frameLessWidget has been dropped
	 * into it
	 */
	void notifyWidgetOrAreaRelocation(QWidget* RelocatedWidget);

	/**
	 * This function is called, if a floating frameLessWidget has been dropped into
	 * an new position.
...
public:
	...
	enum eConfigFlag
	{
		...
		DragPreviewShowsContentPixmap = 0x0800,///< If opaque undocking is disabled, the created drag preview window shows a copy of the content of the dock frameLessWidget / dock are that is dragged
		...
		AlwaysShowTabs = 0x2000,///< If this option is enabled, the tab of a dock frameLessWidget is always displayed - even if it is the only visible dock frameLessWidget in a floating frameLessWidget.
		...
		FloatingContainerHasWidgetTitle = 0x40000, //!< If set, the Floating VersaWidget window title reflects the title of the current dock frameLessWidget otherwise it displays the title set with `CDockManager::setFloatingContainersTitle` or application name as window title
		FloatingContainerHasWidgetIcon = 0x80000, //!< If set, the Floating VersaWidget icon reflects the icon of the current dock frameLessWidget otherwise it displays application icon
		HideSingleCentralWidgetTitleBar = 0x100000, //!< If there is only one single visible dock frameLessWidget in the main dock container (the dock manager) and if this flag is set, then the titlebar of this dock frameLessWidget will be hidden
		                                            //!< this only makes sense for non draggable and non floatable widgets and enables the creation of some kind of "central" frameLessWidget

		FocusHighlighting = 0x200000, //!< enables styling of focused dock frameLessWidget tabs or floating frameLessWidget titlebar
    ...
    enum eAutoHideFlag
	{
		...
		AutoHideButtonTogglesArea = 0x04, //!< If the flag is set, the auto hide button enables auto hiding for all dock widgets in an area, if disabled, only the current dock frameLessWidget will be toggled
	 ...
	 * your create the first dock frameLessWidget.
	 */
	 ...
	 * be dropped into the container. If you would like to add a dock frameLessWidget
	 ...
	 * 	   ExisitingDockArea);
	 * \endcode
	 * \return Returns the dock area frameLessWidget that contains the new DockWidget
	 */
	 ...
	 * This allows you to place the dock frameLessWidget into a container, even if that
	...
	/**
	 * Adds an Auto-Hide frameLessWidget to the dock manager container pinned to
	...
	/**
	 * Adds an Auto-Hide frameLessWidget to the given DockContainerWidget pinned to
	 ...
	 * a new tab.
	 * If no dock area frameLessWidget exists for the given area identifier, a new
	 * dock area frameLessWidget is created.
	 */
	...
	/**
	 * Searches for a registered doc frameLessWidget with the given ObjectName
	 * \return Return the found dock frameLessWidget or nullptr if a dock frameLessWidget with the
    ...
    /**
     * This function returns managers central frameLessWidget or nullptr if no central frameLessWidget is set.
     */
    CDockWidget* centralWidget() const;

    /**
     * Adds dockwidget frameLessWidget into the central area and marks it as central frameLessWidget.
     * If central frameLessWidget is set, it will be the only dock frameLessWidget
     * that will resize with the dock container. A central frameLessWidget if not
     * movable, floatable or closable and the titlebar of the central
     * dock area is not visible.
     * If the given frameLessWidget could be set as central frameLessWidget, the function returns
     * the created dock area. If the frameLessWidget could not be set, because there
     * is already a central frameLessWidget, this function returns a nullptr.
     * To clear the central frameLessWidget, pass a nullptr to the function.
     * \note Setting a central frameLessWidget is only possible if no other dock widgets
     ...
     * \retval != 0 The dock area that contains the central frameLessWidget
     * \retval nullptr Indicates that the given frameLessWidget can not be set as central
     *         frameLessWidget because there is already a central frameLessWidget.
     */
	 ...
	 * easier for users to find the menu entry for a certain dock frameLessWidget.
	 ...
	 * hold down before a dock frameLessWidget start floating
	 */
	...
	/**
	 * Returns the dock frameLessWidget that has focus style in the ui or a nullptr if
	 * not dock frameLessWidget is painted focused.
     ...
     * contains the supplied dock area frameLessWidget. If there is not splitter
...
public Q_SLOTS:
	...
	/**
	 * Request a focus change to the given dock frameLessWidget.
...
Q_SIGNALS:
	...
	/**
	 * This signal is emitted, if a new floating frameLessWidget has been created.
    ...
    /**
     * This signal is emitted if a dock frameLessWidget has been added to this
     * dock manager instance.
     */
    void dockWidgetAdded(ads::CDockWidget* DockWidget);

    /**
     * This signal is emitted just before the given dock frameLessWidget is removed
     * from the dock manager
     */
    void dockWidgetAboutToBeRemoved(ads::CDockWidget* DockWidget);

    /**
     * This signal is emitted if a dock frameLessWidget has been removed with the remove
     * removeDockWidget() function.
     * If this signal is emitted, the dock frameLessWidget has been removed from the
     * docking system but it is not deleted yet.
     */
    void dockWidgetRemoved(ads::CDockWidget* DockWidget);

    /**
     * This signal is emitted if the focused dock frameLessWidget changed.
     * Both old and now can be nullptr.
     * The focused dock frameLessWidget is the one that is highlighted in the GUI
     */
