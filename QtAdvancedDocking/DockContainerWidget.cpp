...
enum eDropMode
{
	DropModeIntoArea,///< drop frameLessWidget into a dock area
...
public:
	...
	/**
	 * Adds dock frameLessWidget to container and returns the dock area that contains
	 * the inserted dock frameLessWidget
	 */
	CDockAreaWidget* addDockWidgetToContainer(DockWidgetArea area, CDockWidget* Dockwidget);

	/**
	 * Adds dock frameLessWidget to a existing DockWidgetArea
	 */
	...
	/**
	 * Moves the dock frameLessWidget or dock area given in VersaWidget parameter to
	 ...
	 * \param[out] CreatedWidget The frameLessWidget created from parsed data or 0 if
	 * the parsed frameLessWidget was an empty splitter
	...
	/**
	 * Ensures equal distribution of the sizes of a splitter if an dock frameLessWidget
    ...
    /**
     * This function forces the dock container frameLessWidget to update handles of splitters
     * based if a central frameLessWidget exists.
     */
    void updateSplitterHandles(QSplitter* splitter);

    /**
     * If no central frameLessWidget exists, the widgets resize with the container.
     * If a central frameLessWidget exists, the widgets surrounding the central frameLessWidget
	...
	DockWidgetArea area)
{
	...
	// Now we can insert the floating frameLessWidget content into this container
	...
	// If we dropped the floating frameLessWidget into the main dock container that does
	...
	CFloatingDockContainer* FloatingWidget, CDockAreaWidget* TargetArea, int TabIndex)
{
	...
	// If the floating frameLessWidget contains only one single dock are, then the
	// current dock frameLessWidget of the dock area will also be the future current
	// dock frameLessWidget in the drop area.
	if (TopLevelDockArea)
	{
	...
	for (int i = 0; i < NewDockWidgets.count(); ++i)
	{
		...
		// If the floating frameLessWidget contains multiple visible dock areas, then we
		// simply pick the first visible open dock frameLessWidget and make it
		// the current one.
		...
		CDockAreaWidget* TargetArea, DockWidgetArea area, int TabIndex)
{
	...
	// frameLessWidget will become tabs of the drop area
...
void DockContainerWidgetPrivate::moveToContainer(QWidget* Widget, DockWidgetArea area)
{
	...
	{
		// We check, if we insert the dropped frameLessWidget into the same place that
...
void DockContainerWidgetPrivate::addDockAreasToList(const QList<CDockAreaWidget*> NewDockAreas)
{
	...
	// If the user dropped a floating frameLessWidget that contains only one single
	...
	// is invisible, if the dock are is a single dock area in a floating frameLessWidget.
	if (1 == CountBefore)
	{
	...
	QWidget*& CreatedWidget, bool Testing)
{
	...
	while (s.readNextStartElement())
	{
		if (s.name() != QLatin1String("VersaWidget"))
		{
			continue;
		}
...
{
#if defined(QT_DEBUG)
	QSplitter* Splitter = qobject_cast<QSplitter*>(frameLessWidget);
	QByteArray buf;
    buf.fill(' ', level * 4);
	if (Splitter)
	{
		...
		for (int i = 0; i < Splitter->count(); ++i)
		{
			dumpRecursive(level + 1, Splitter->frameLessWidget(i));
		}
	}
	else
	{
		CDockAreaWidget* DockArea = qobject_cast<CDockAreaWidget*>(frameLessWidget);
		if (!DockArea)
		{
			return;
		}
	...
	CDockAreaWidget* DockAreaWidget, int Index)
{
	...
	if (TopLevelDockWidget)
	{
		...
		// If the container contained only one visible dock frameLessWidget, the we need
		// to emit a top level event for this frameLessWidget because it is not the one and
		// only visible docked frameLessWidget anymore
...
void CDockContainerWidget::removeDockArea(CDockAreaWidget* area)
{
	...
	if (Splitter == d->RootSplitter)
	{
		...
		// If the one and only content frameLessWidget of the splitter is not a splitter
...
emitAndExit:
	...
	// Updated the title bar visibility of the dock frameLessWidget if there is only
    // one single visible dock frameLessWidget
...
int CDockContainerWidget::visibleDockAreaCount() const
{
	...
	// movement of floating frameLessWidget
	...
	const QPoint& TargetPos)
{
	...
	// them into this frameLessWidget
	...
	{ 
		...
		// If we dropped a floating frameLessWidget with only one single dock frameLessWidget, then we
		// drop a top level frameLessWidget that changes from floating to docked now
		CDockWidget::emitTopLevelEventForWidget(SingleDroppedDockWidget, false);

		// If there was a top level frameLessWidget before the drop, then it is not top
		// level frameLessWidget anymore
...
bool CDockContainerWidget::restoreState(CDockingStateReader& s, bool Testing)
{
	...
	if (IsFloating)
	{
        ADS_PRINT("Restore floating frameLessWidget");
