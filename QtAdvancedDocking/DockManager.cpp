	...
	bool Testing)
{
    ...
    if (CentralWidget)
    {
		...
		// If we have a central frameLessWidget but a state without central frameLessWidget, then
		// something is wrong.
		if (CentralWidgetAttribute.isEmpty())
		{
			qWarning() << "Dock manager has central frameLessWidget but saved state does not have central frameLessWidget.";
			return false;
		}

		// If the object name of the central frameLessWidget does not match the name of the
		// saved central frameLessWidget, the something is wrong
		if (CentralWidget->objectName() != CentralWidgetAttribute.toString())
		{
			qWarning() << "Object name of central frameLessWidget does not match name of central frameLessWidget in saved state.";
			return false;
		}
    }
...
void DockManagerPrivate::restoreDockWidgetsOpenState()
{
    ...
    for (auto DockWidget : DockWidgetsMap)
    {
    	...
    	{
    		// If the DockWidget is an auto hide frameLessWidget that is not assigned yet,
...
bool CDockManager::restoreState(const QByteArray &state, int version)
{
	...
	// which in turn  means, that each time a frameLessWidget is removed the stack
	// will show and raise the next available frameLessWidget which in turn
...
void CDockManager::showEvent(QShowEvent *event)
{
	...
	{
		// Check, if someone closed a floating dock frameLessWidget before the dock
		// manager is shown
...
CDockAreaWidget* CDockManager::setCentralWidget(CDockWidget* widget)
{
	...
	// Setting a new central frameLessWidget is now allowed if there is already a central
	// frameLessWidget or if there are already other dock widgets
	if (d->CentralWidget)
	{
		qWarning("Setting a central frameLessWidget not possible because there is already a central frameLessWidget.");
		return nullptr;
	}

	// Setting a central frameLessWidget is now allowed if there are already other
	// dock widgets.
	if (!d->DockWidgetsMap.isEmpty())
	{
		qWarning("Setting a central frameLessWidget not possible - the central frameLessWidget need to be the first "
			"dock frameLessWidget that is added to the dock manager.");
		return nullptr;
	}
...
void CDockManager::hideManagerAndFloatingWidgets()
{
	...
	for (auto FloatingWidget : d->FloatingWidgets)
	{
		if ( FloatingWidget->isVisible() )
		{
			...
			// save as floating frameLessWidget to be shown when CDockManager will be shown back
			d->HiddenFloatingWidgets.push_back( FloatingWidget );
			FloatingWidget->hide();

			// hiding floating frameLessWidget automatically marked contained CDockWidgets as hidden
