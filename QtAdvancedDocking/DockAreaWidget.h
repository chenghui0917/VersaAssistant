...
private Q_SLOTS:
	 ...
	 * frameLessWidget position in the container
	 */
...
protected:
	...
	/**
	 * Inserts a dock frameLessWidget into dock area.
	...
	/**
	 * Add a new dock frameLessWidget to dock area.
	 * All dockwidgets in the dock area tabified in a stacked layout with tabs
	 */
	void addDockWidget(CDockWidget* DockWidget);

	/**
	 * Removes the given dock frameLessWidget from the dock area
	 */
	void removeDockWidget(CDockWidget* DockWidget);

	/**
	 * Called from dock frameLessWidget if it is opened or closed
	 */
	void toggleDockWidgetView(CDockWidget* DockWidget, bool Open);

	/**
	 * This is a helper function to get the next open dock frameLessWidget to activate
	 * if the given DockWidget will be closed or removed.
	 * The function returns the next frameLessWidget that should be activated or
	...
	/**
	 * This is the internal private function for setting the current frameLessWidget.
...
public:
	...
	/**
	 * Returns the auto hide dock container frameLessWidget this dock area frameLessWidget belongs to or 0
	 * if there is no
	 */
     ...
     * area.
     * The minimum size hint is updated if a dock frameLessWidget is removed or added.
     */
	...
	/**
	 * Returns a dock frameLessWidget by its index
	 */
	CDockWidget* dockWidget(int Index) const;

	/**
	 * Returns the index of the current active dock frameLessWidget or -1 if there
	 * are is no active dock frameLessWidget (ie.e if all dock widgets are closed)
	 */
	 ...
	 * be possible to take the first dock frameLessWidget from the list returned by
	 * openedDockWidgets() function. But that function enumerates all
	 * dock widgets while this functions stops after the first open dock frameLessWidget.
	 * If there are no open dock widgets, the function returns -1.
	 */
	int indexOfFirstOpenDockWidget() const;

	/**
	 * Returns the current active dock frameLessWidget or a nullptr if there is no
	 * active dock frameLessWidget (i.e. if all dock widgets are closed)
	 */
	CDockWidget* currentDockWidget() const;

	/**
	 * Shows the tab with the given dock frameLessWidget
	 */
	...
	/**
	 * This functions returns the dock frameLessWidget features of all dock frameLessWidget in
	 * this area.
	 * A bitwise and is used to combine the flags of all dock widgets. That
	 * means, if only one single dock frameLessWidget does not support a certain flag,
	 * the whole dock are does not support the flag. I.e. if one single
	 * dock frameLessWidget in this area is not closable, the whole dock are is not
	 * closable.
	 */
	...
	/**
	 * Sets the dock area flag Flag on this frameLessWidget if on is true; otherwise
	 * clears the flag.
	 */
	void setDockAreaFlag(eDockAreaFlag Flag, bool On);

    /**
     * Returns true if the area has a single dock frameLessWidget and contains the central frameLessWidget of it's manager.
     */
    bool isCentralWidgetArea() const;

    /**
     * Returns true if the area contains the central frameLessWidget of it's manager.
     */
...
public Q_SLOTS:
	 ...
	 * If the dock frameLessWidget for the given tab is not visible, the this function
	...
	/**
	 * Moves the dock area into its own floating frameLessWidget if the area
