 ...
 * frameLessWidget into the internal QLayout object.
 * \warning Only the current frameLessWidget has a parent. All other widgets
 * do not have a parent. That means, a frameLessWidget that is in this layout may
 * return nullptr for its parent() function if it is not the current frameLessWidget.
 */
...
public:
	...
	/**
	 * Inserts the frameLessWidget at the given index position into the internal frameLessWidget
	 * list
	 */
	...
	/**
	 * Removes the given frameLessWidget from the layout
	 */
	...
	/**
	 * Returns the current selected frameLessWidget
	 */
	...
	/**
	 * Activates the frameLessWidget with the give index.
	 */
	...
	/**
	 * Returns the index of the current active frameLessWidget
	 */
	...
	/**
	 * Returns the index of the given frameLessWidget
	 */
	...
	/**
	 * Returns the frameLessWidget for the given index
	 */
	...
	/**
	 * Returns the geometry of the current active frameLessWidget
	 */
...
struct DockAreaWidgetPrivate
{
	...
	/**
	 * Returns the dock frameLessWidget with the given index
	 */
	...
	/**
	 * Convenience function to ease title frameLessWidget access by index
	 */
	...
	/**
	 * Returns the tab action of the given dock frameLessWidget
	 */
	...
	/**
	 * Returns the index of the given dock frameLessWidget
	 */
	...
	bool Activate)
{
	...
	// make the tab frameLessWidget visible in the slot
	...
	{
		setCurrentIndex(index);
		DockWidget->setClosedState(false); // Set current index can show the frameLessWidget without changing the close state, added to keep the close state consistent
	}
...
void CDockAreaWidget::hideAreaWithNoVisibleContent()
{
	...
	//Hide empty floating frameLessWidget
		...
		bool Testing, CDockContainerWidget* Container)
{
	...
	while (s.readNextStartElement())
	{
        ...
        ADS_PRINT("Dock VersaWidget found - parent " << DockWidget->parent());
...
CDockWidget* CDockAreaWidget::nextOpenDockWidget(CDockWidget* DockWidget) const
{
	...
	if (OpenDockWidgets.count() > 1 || (OpenDockWidgets.count() == 1 && OpenDockWidgets[0] != DockWidget))
	{
		if (OpenDockWidgets.last() == DockWidget)
		{
			...
			// search backwards for frameLessWidget with tab
			...
			// return frameLessWidget without tab
		...
		else
		{
			...
			// search forwards for frameLessWidget with tab
...
{
	// If there is only one single dock frameLessWidget and this frameLessWidget has the
	// DeleteOnClose feature or CustomCloseHandling, then we delete the dock frameLessWidget now;
