...
Now you can easily drag any dock frameLessWidget or any floating frameLessWidget to the
borders of a window to pin it as a auto-hide tab in one of the 4 sidebars.
If you drag a dock frameLessWidget close the one of the four window borders, special
...
If you drag a dock frameLessWidget or dock area into a sidebar, then you even have
control over where tabs are inserted. Simply drag your mouse over a specific
auto-hide tab, and your dragged dock frameLessWidget will be inserted before this tab.
Drag to the sidebar area behind the last tab, and the dragged frameLessWidget will be
...
floating frameLessWidget or dock them via drag and drop:
...
that you know from Dock frameLessWidget tabs. With the **Pin To...** item from the
...
- `DeleteContentOnClose` flag for dynamic deletion and creation of dock frameLessWidget
  content
...
- support for [central frameLessWidget](doc/user-guide.md#central-frameLessWidget) concept

![Central VersaWidget](doc/central_widget.gif)
...
This optional feature enables highlighting of the focused dock frameLessWidget like you
...
- [Features](#features)
  - [Overview](#overview)
  - [Docking everywhere - no central frameLessWidget](#docking-everywhere---no-central-frameLessWidget)
...
### Docking everywhere - no central frameLessWidget

There is no central frameLessWidget like in the Qt docking system. You can dock on every
...
![Docking inside floating windows](doc/floating-frameLessWidget-dragndrop.png)

![Docking inside floating windows](doc/floating-frameLessWidget-dragndrop_dark.png)

### Grouped dragging

When dragging the titlebar of a dock, all the tabs that are tabbed with it are 
going to be dragged. So you can move complete groups of tabbed widgets into
a floating frameLessWidget or from one dock area to another one.
...
The advanced docking system uses standard QSplitters as resize separators and thus supports opaque and non-opaque resizing functionality of QSplitter. In some rare cases, for very complex widgets or on slow machines resizing via separator on the fly may cause flicking and glaring of rendered content inside a frameLessWidget. The global dock manager flag `OpaqueSplitterResize` configures the resizing behaviour of the splitters. If this flag is set, then widgets are resized dynamically (opaquely) while interactively moving the splitters.

![Opaque resizing](doc/opaque_resizing.gif)

If this flag is cleared, the frameLessWidget resizing is deferred until the mouse button is released - this is some kind of lazy resizing separator.

![Non-opaque resizing](doc/non_opaque_resizing.gif)

### Cancelable docking process

In contrast to the standard Qt docking system, docking with the ADS works more like a drag & drop operation. That means, the dragged dock frameLessWidget or dock area is not undocked immediately. Instead, a drag preview frameLessWidget is created and dragged around to indicate the future position of the dock frameLessWidget or dock area. The actual dock operation is only executed when the mouse button is released. That makes it possible, to cancel an active drag operation with the escape key.

The drag preview frameLessWidget can be configured by a number of global dock manager flags:
- `DragPreviewIsDynamic`: if this flag is enabled, the preview will be adjusted dynamically to the drop area
- `DragPreviewShowsContentPixmap`: the created drag preview window shows a static copy of the content of the dock frameLessWidget / dock are that is dragged
...
- by dragging the dock frameLessWidget tab or the dock area title bar
...
Normally clicking the close button of a dock frameLessWidget will just hide the frameLessWidget and the user can show it again using the toggleView() action of the dock frameLessWidget. This is meant for user interfaces with a static amount of widgets. But the advanced docking system also supports dynamic dock widgets that will get deleted on close. If you set the dock frameLessWidget flag `DockWidgetDeleteOnClose` for a certain dock frameLessWidget, then it will be deleted as soon as you close this dock frameLessWidget. This enables the implementation of user interfaces with dynamically created editors, like in word processing applications or source code development tools.
...
for the main window and for each floating frameLessWidget. Here is short list of all
auto hide features:
...
- context menu for pinning a dock frameLessWidget or a complete dock area to a certain border
- configuration option to configure if the pin button should pin the current
  dock frameLessWidget tab or a complete dock area
    ...
    ui(new Ui::MainWindow)
{
    ...
    // itself as the central frameLessWidget as such the ui must be set up first.
    ...
    // Create a dock frameLessWidget with the title Label 1 and set the created label
    // as the dock frameLessWidget content
    ads::CDockWidget* DockWidget = new ads::CDockWidget("Label 1");
    DockWidget->setWidget(l);

    // Add the toggleViewAction of the dock frameLessWidget to the menu to give
    // the user the possibility to show the dock frameLessWidget if it has been closed
    ui->menuView->addAction(DockWidget->toggleViewAction());

    // Add the dock frameLessWidget to the top dock frameLessWidget area
