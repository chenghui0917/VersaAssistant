 ...
 * undocking is used, the a real CFloatingDockContainer frameLessWidget will be created
 */
...
public:
	...
	/**
	 * Moves the frameLessWidget to a new position relative to the position given when
	 * startFloating() was called.
	 * This function should be called from a mouse mouve event handler to
	 * move the floating frameLessWidget on mouse move events.
	 */
	virtual void moveFloating() = 0;

	/**
	 * Tells the frameLessWidget that to finish dragging if the mouse is released.
...
/**
 * This implements a floating frameLessWidget that is a dock container that accepts
...
protected:
	 ...
	 * Use moveToGlobalPos() to move the frameLessWidget to a new position
	...
	/**
	 * Call this function to start dragging the floating frameLessWidget
	 */
	 ...
	 * and size of the floating frameLessWidget
	 */
...
public:
	...
	/**
	 * Create empty floating frameLessWidget - required for restore state
	 */
	CFloatingDockContainer(CDockManager* DockManager);

	/**
	 * Create floating frameLessWidget with the given dock area
	 */
	CFloatingDockContainer(CDockAreaWidget* DockArea);

	/**
	 * Create floating frameLessWidget with the given dock frameLessWidget
	 */
    ...
    /**
     * This function returns true, if this floating frameLessWidget has only one single
     * visible dock frameLessWidget in a single visible dock area.
     * The single dock frameLessWidget is a real top level floating frameLessWidget because no
     * other widgets are docked.
     */
    bool hasTopLevelDockWidget() const;

    /**
     * This function returns the first dock frameLessWidget in the first dock area.
     * If the function hasSingleDockWidget() returns true, then this function
     * returns this single dock frameLessWidget.
     */
    CDockWidget* topLevelDockWidget() const;

    /**
     * This function returns a list of all dock frameLessWidget in this floating frameLessWidget.
     * This is a simple convenience function that simply calls the dockWidgets()
     * function of the internal container frameLessWidget.
     */
	...
	/**
	 * Returns true if the floating frameLessWidget has a native titlebar or false if
	 * the floating frameLessWidget has a QWidget based title bar
	 */
