...
struct AutoHideDockContainerPrivate
{
	...
	/**
	 * Convenience function to get a dock frameLessWidget area
	 */
    ...
    d(new AutoHideDockContainerPrivate(this))
{
	...
	// dock frameLessWidget. If you add it to the layout before it contains the dock frameLessWidget
...
void CAutoHideDockContainer::addDockWidget(CDockWidget* DockWidget)
{
	if (d->DockWidget)
	{
		// Remove the old dock frameLessWidget at this area
...
{
	cleanupAndDelete();
	// If we unpin the auto hide dock frameLessWidget, then we insert it into the same
	// location like it had as a auto hide frameLessWidget.  This brings the least surprise
	// to the user and he does not have to search where the frameLessWidget was inserted.
...
bool CAutoHideDockContainer::eventFilter(QObject* watched, QEvent* event)
{
	...
	else if (event->type() == QEvent::MouseButtonPress)
	{
		...
		// Ignore non frameLessWidget events
		if (!widget)
		{
		...
		// user clicked into container - collapse the auto hide frameLessWidget
		collapseView(true);
	}
    else if (event->type() == internal::FloatingWidgetDragStartEvent)
    {
    	// If we are dragging our own floating frameLessWidget, the we do not need to
    	// collapse the view
	...
	int TabIndex)
{
	...
	// then we resize it to the original dock frameLessWidget size, to avoid
	// an extremely stretched dock frameLessWidget after insertion
