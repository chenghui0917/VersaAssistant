...
struct FloatingDragPreviewPrivate
{
	...
	/**
	 * Creates the real floating frameLessWidget in case the mouse is released outside
...
void FloatingDragPreviewPrivate::updateDropOverlays(const QPoint &GlobalPos)
{
	...
	// Include the overlay frameLessWidget we're dragging as a visible frameLessWidget
	...
	d(new FloatingDragPreviewPrivate(this))
{
	...
	// Create a static image of the frameLessWidget that should get undocked
...
void CFloatingDragPreview::finishDragging()
{
	...
	else if (ContainerDropArea != InvalidDockWidgetArea)
	{
		...
		// then we tabify the dropped frameLessWidget into the only visible dock area
...
void CFloatingDragPreview::cleanupAutoHideContainerWidget(DockWidgetArea ContainerDropArea)
{
	...
	// If the dropped frameLessWidget is already an auto hide frameLessWidget and if it is moved
