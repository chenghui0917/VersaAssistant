...
def write_qrc(SourceName):
    ...
    for FolderName in ListFolder:
        ...
        for File in ListFile:
            ...
            os.rename(SourcePath+"\\"+FolderName+"\\"+File, SourcePath+"\\"+FolderName+"\\"+item_new)  # 重命名
            renameTotal +=1
    ...
    pFile.write(u'<!DOCTYPE RCC>\n<RCC version="1.0">\n\t<qresource prefix="/">\n')
    for FolderName in ListFolder:
        ListFile = os.listdir(SourcePath+"\\"+FolderName)
        for File in ListFile:
            # print(u'<file>' + SourceName +'/' + FolderName + '/' + File + '</file>\n')
            pFile.write(u'\t\t<file>' + SourceName +'/' + FolderName + '/' + File + '</file>\n')
    pFile.write(u'</qresource>\n</RCC>')
    pFile.close()
